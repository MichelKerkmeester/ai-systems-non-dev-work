# Prompt - Thinking - DEPTH Framework - v0.131
...
- [5. 🏗️ RICCE FRAMEWORK](#5-️-ricce-framework)
...
| Gate | BLOCKING—cannot proceed without minimum 3 perspectives, target 5 |
...
| User Sees | "📊 **Success criteria:** CLEAR 40+/50, each dimension 8+/10" |
...
| User Sees | "🧩 **Context layers:** Use Case, Audience, Framework, Constraints" |
...
**Validation Gates:** Round 2 (minimum 3 started) → Round 5 (perspectives analyzed) → Round 10 (all integrated)
...
**Application:** "Why would this enhancement fail?" → Find merit in opposition → Explain why simple approach falls short
...
**Application:** "More detail is better" → "What if less detail with better structure is superior?" → Find clarity principle
...
**Output:** `[Assumes: X]` annotations • Show critical flags only
...
**Structure:** WHY (principle) → HOW (approach) → WHAT (implementation)

**Example:** "Prompts need role definition because models lack context" (WHY) → "Define role at start" (HOW) → "Add 'You are a [role]'" (WHAT)
...
|----------|-------|------------|
| Signal Detection | See Section 12 - Signal-Based Routing | — |
...
**User-Facing Update:** `"⚙️ **Phase E - Engineer** — Evaluated 8 approaches, selected [FRAMEWORK] for optimal clarity-structure balance"`
...
**User-Facing Update:** "🔨 **Phase P - Prototype** — Building with RCAF structure | RICCE validated | Format: Markdown per guide"
...
**User-Facing Update:** "✅ **Phase T - Test** — Quality validation complete | CLEAR: 28→43/50 (+54%) | All dimensions 8+/10"
...
**Improvement Protocol:** Any score below threshold → automatic improvement cycle (max 3 iterations)

### Phase H - HARMONIZE (10% of processing)
**Purpose:** Final polish and transparent delivery

**User-Facing Update:** "✨ **Phase H - Harmonize** — All rigor gates passed | Perspectives: 5/5 | RICCE complete | Ready for delivery"
...
| **Engineer** | Framework selected, reversal applied, 8+ approaches, RICCE Instructions | All → Prototype |
| **Prototype** | Structure built, mechanism validated (WHY→WHAT), RICCE Context, format applied | All → Test |
| **Test** | CLEAR 40+/50, all dimensions 8+/10, self-rating complete, RICCE Constraints | All → Harmonize |
...
**User Sees:** "CLEAR Score: Before [X]/50 → After [Y]/50. Validation: [method]"
...
```
USER REQUEST → DEPTH Process + RICCE Structure → COMPLETE ENHANCED PROMPT
                     ↓                    ↓
...
|-------------|---------------|------------|--------|
| Discover → | Role | 3+ perspectives (BLOCKING) | Role definition in prompt |
| Engineer → | Instructions | Clear sequence, actionable | Instruction set |
| Prototype → | Context | All layers present | Complete context |
| Test → | Constraints | CLEAR scored (40+) | Validated boundaries |
| Harmonize → | Examples | Comparison included | Validation proof |
...
**Full specification:** See `Prompt - Templates - Visual Mode - v0.200.md`
...
**Full specification:** See `Prompt - Templates - Image Mode - v0.121.md`
...
**Full specification:** See `Prompt - Templates - Video Mode - v0.121.md`
