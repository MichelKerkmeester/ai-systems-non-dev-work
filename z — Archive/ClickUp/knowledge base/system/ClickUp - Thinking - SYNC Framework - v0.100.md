...
-  **Sprint/Standup:** CLI-only — `cu sprint` and `cu summary` have NO MCP equivalent
-  **Documents/Goals/Webhooks:** MCP-only — no CLI support for enterprise features
-  **Time Tracking:** MCP-only (`manage_time_entries` with 10+ actions)
-  **Bulk Operations:** MCP-only (`create_bulk_tasks`, `update_bulk_tasks`)
-  **CLI Rate Limit:** ClickUp API rate limits apply to both CLI and MCP
-  **Node.js Requirement:** CLI requires Node.js >= 22.0.0
...
| **Y**ield | Tool selection + Fallback planning | " Yielding (CLI selected: cu commands, MCP fallback ready)" |
...
| **Y**ield | Select tool, plan fallback | " Yielding (CLI selected for task ops)" |
