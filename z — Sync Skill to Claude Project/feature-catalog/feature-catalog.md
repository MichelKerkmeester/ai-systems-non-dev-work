...
The command requires a registered `--system`. Report-only invocation explains whether a valid manifest exists, while `--write` renders changed mirrors and generated regions through staged atomic operations. `--recover` rolls back a detected interrupted transaction.
...
The check expands included files and directories, removes excluded paths and blocks files that are covered but missing from the manifest mirror list. It checks every mirror source, rejects broken or repository-escaping symlinks and rejects mirror targets that collide by case.
...
The check reports undeclared, missing and count-mismatched knowledge files. It compares each ordinary mirror with its dereferenced source and skips only targets declared in `derivationExceptions`. Unknown knowledge files are reported and never deleted automatically.
...
Verifies that recorded package-lock hashes still describe the live kernel and mirror bytes.

#### Current Reality

When `package-lock.json` exists, the check validates each `sha16` as the prefix of its own `sha256` and compares recorded hashes with current files. Missing files remain the responsibility of the inventory checks.
...
The check requires every declared target and section marker to exist. When a region hash exists in `package-lock.json`, a changed body produces `REGION_DRIFT`. Regions without a prior lock record remain unreported until a sync records their bytes.
...
The transaction engine stages all write bytes before touching live targets, validates staged content, rehashes sources, acquires the fleet lock and journals every operation. It writes the package lock last, then removes backups and the journal after success.
...
The CLI refuses a new sync when a journal exists. The recovery path reverses journaled operations, removes staged and backup files, deletes the journal and never forward-completes an unknown crash point.
...
The registry requires schema version 1, exactly ten entries, unique identifiers and roots, required path fields and the fixed kernel path. It rejects missing or unexpected system identifiers rather than discovering directories dynamically.
...
The hand-rolled validator enforces identity, fixed paths, coverage, mirrors, contract inputs, validators, retired-name settings, counts and generated regions against the documented schema. A `derivationExceptions` entry requires a path and reason, and the sync and parity paths honor that declared waiver.
